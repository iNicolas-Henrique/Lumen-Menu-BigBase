...
namespace YimMenu::Features
{
	...
	BoolCommand _LogTenebrisPackets("logterminuspackets", "Registrar pacotes do Tenebris", "Registra os pacotes P2P do Tenebris");
...
namespace YimMenu::Hooks
{
	...
	bool IsP2pTenebris(void* buffer, int sz)
	{
		auto flags = GetFlags(buffer, sz);
	...
	bool Protections::HandlePeerRelayPacket(__int64 _this, __int64 netSockFrom, void* buffer, unsigned int sizeOfBuffer)
	{
		...
		else if (Features::_LogTenebrisPackets.GetState() && IsP2pTenebris(buffer, sizeOfBuffer))
		{
			auto packet = (unsigned char*)buffer;
			LOG(INFO) << "Pacote P2P do Tenebris: " << BytesToHexStr(packet, sizeOfBuffer) << " of size " << sizeOfBuffer;
		}
