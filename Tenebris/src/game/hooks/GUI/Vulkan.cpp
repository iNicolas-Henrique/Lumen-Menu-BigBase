...
namespace YimMenu::Hooks
{
    VkResult VKAPI_CALL Vulkan::QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
    {
        ...
        if (VkSemaphore overlayComplete = Renderer::VkOnPresent(queue, pPresentInfo))
        {
            // The overlay submission consumes the game's present semaphores. Present must
            // therefore wait on the semaphore produced by that submission, not consume them again.
            waits.push_back(overlayComplete);
            presentInfo.waitSemaphoreCount = static_cast<uint32_t>(waits.size());
            presentInfo.pWaitSemaphores = waits.data();
        }

        return BaseHook::Get<Vulkan::QueuePresentKHR, DetourHook<decltype(&QueuePresentKHR)>>()->Original()(queue, &presentInfo);
    }
    ...
    {
        const VkResult result = BaseHook::Get<Vulkan::CreateSwapchainKHR, DetourHook<decltype(&CreateSwapchainKHR)>>()->Original()(device, pCreateInfo, pAllocator, pSwapchain);
        if (result == VK_SUCCESS && pCreateInfo)
        {
            Renderer::SetResizing(true);
            Renderer::VkCleanupRenderTarget();
            Renderer::VkSetDevice(device);
            Renderer::VkSetSwapchainInfo(pCreateInfo->imageFormat, pCreateInfo->imageExtent, pCreateInfo->minImageCount);
