...
{
	inline void ShowTeleportNotice(std::string messagePt, std::string messageEn, NotificationType type)
	{
		Notifications::Show(Localization::Text("Teleporte"),
		    Localization::IsPortuguese() ? std::move(messagePt) : std::move(messageEn),
		    type,
		    5000,
		    nullptr,
		    "",
	...
	inline bool TeleportEntity(int ent, rage::fvector3 coords, bool loadGround = false)
	{
		...
		if (loadGround)
		{
			...
			{
				ShowTeleportNotice("Não foi possível encontrar um ponto seguro no solo.", "Could not find a safe ground position.", NotificationType::Error);
				return false;
			}
		...
		ShowTeleportNotice("Teleporte concluído.", "Teleport completed.", NotificationType::Success);
		return true;
	}
	...
	inline bool WarpIntoVehicle(int ped, int veh)
	{
		...
		if (seat < -1)
		{
			ShowTeleportNotice("Não há assentos livres no veículo.", "There are no free seats in the vehicle.", NotificationType::Error);
			return false;
		}
	...
	inline bool TeleportPlayerToCoords(Player player, Vector3 coords)
	{
		...
		{
			ShowTeleportNotice("O jogador selecionado está morto e não pode ser teleportado.", "The selected player is dead and cannot be teleported.", NotificationType::Error);
			return false;
		}
		...
		{
			ShowTeleportNotice("Não foi possível preparar o objeto de rede usado pelo teleporte.", "Could not prepare the network object used by the teleport.", NotificationType::Error);
			return false;
		}
		...
		ent.ForceControl();
		ent.Delete();
		std::erase_if(g_SpoofingStorage.m_RemotePlayerTeleports, [vehId](auto& obj) { return obj.first == vehId; });
		ShowTeleportNotice("Teleporte concluído.", "Teleport completed.", NotificationType::Success);
		return true;
	}
}
