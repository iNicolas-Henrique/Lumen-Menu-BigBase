...
> O Lumen possui eventos de notoriedade/press charges, mas eles não fornecem um
...
`src/game/rdr/invoker/natives.json` já versionado no Lumen.
...
| Área | Native candidata | Possível opção no Lumen |
|---|---|---|
...
- O campo `bountyamount` agora aceita entrada digitada até `INT_MAX`; o limite
  antigo de 150.000 centavos era do Lumen, não estava documentado na assinatura
...
- Portanto, nenhuma opção de sessão solo foi adicionada nesta etapa. O Lumen já
