...
namespace YimMenu::Features
{
	...
	BoolCommand _LogLumenPackets("logterminuspackets", "Registrar pacotes do Lumen", "Registra os pacotes P2P do Lumen");
...
namespace YimMenu::Hooks
{
	...
	bool IsP2pLumen(void* buffer, int sz)
	{
		auto flags = GetFlags(buffer, sz);
	...
	bool Protections::HandlePeerRelayPacket(__int64 _this, __int64 netSockFrom, void* buffer, unsigned int sizeOfBuffer)
	{
		...
		else if (Features::_LogLumenPackets.GetState() && IsP2pLumen(buffer, sizeOfBuffer))
		{
			auto packet = (unsigned char*)buffer;
			LOG(INFO) << "Pacote P2P do Lumen: " << BytesToHexStr(packet, sizeOfBuffer) << " of size " << sizeOfBuffer;
		}
