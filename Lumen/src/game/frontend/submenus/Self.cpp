...
namespace YimMenu::Submenus
{
	...
	static std::string GetLumenRoamingPath(const std::string& filename)
	{
		const char* appData = std::getenv("APPDATA");
		std::string base;
		if (appData)
			base = std::string(appData) + "\\Lumen\\";
		else
			base = "Lumen\\";
	...
	{
		std::ofstream out(GetLumenRoamingPath("animation_history.txt"), std::ios::trunc);
	...
	{
		std::ofstream out(GetLumenRoamingPath("animation_favorites.txt"), std::ios::trunc);
	...
	{
		std::ofstream out(GetLumenRoamingPath("music_history.txt"), std::ios::trunc);
	...
	void RenderAnimationsCategory()
	{
		...
		if (!favoriteMusics.empty())
		{
			...
			const char* preview =
			    (favMusicIndex >= 0 && favMusicIndex < (int)favMusicLabels.size()) ? favMusicLabels[favMusicIndex].c_str() : "Select Favorite";
