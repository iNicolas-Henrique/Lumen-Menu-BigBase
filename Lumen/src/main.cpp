...
namespace YimMenu
{
	...
	{
		const auto documents = std::filesystem::path(std::getenv("appdata")) / "Lumen";
		FileMgr::Init(documents);

		LogHelper::Init("Lumen", FileMgr::GetProjectFile("./cout.log"));
		LOG(INFO) << R"(
...
|_____| \___/|_|  |_|_____|_| \_|
		...
		Notifications::Show("Lumen", "Carregado com sucesso", NotificationType::Success);
