...
namespace YimMenu
{
	...
	void AdvancedEditor::Draw()
	{
		...
		const bool detailedPlayerPanel = g_Item->GetMenuLabel() == "Informacoes do jogador";
		const float desiredHeight = detailedPlayerPanel ? 360.0f : 240.0f;
		...
		if (ImGui::Begin("##LumenAdvancedEditor", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings))
		{
			...
			ImGui::Spacing();
			if (ImGui::BeginChild("##LumenAdvancedContent", ImVec2(0.0f, 0.0f), false, ImGuiWindowFlags_AlwaysVerticalScrollbar))
			{
