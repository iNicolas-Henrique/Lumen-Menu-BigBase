...
namespace YimMenu
{
	...
	public:
		...
		static VkSemaphore VkOnPresent(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
		{
			if (!queue)
			{
			...
			return GetInstance().VkOnPresentImpl(queue, pPresentInfo);
		}
		...
		static void VkSetSwapchainInfo(VkFormat format, VkExtent2D extent, uint32_t minImageCount)
		{
			GetInstance().m_VkFormat = format;
			GetInstance().m_VkImageExtent = extent;
			GetInstance().m_VkMinImageCount = minImageCount;
		}
	...
	private:
		...
		VkSemaphore VkOnPresentImpl(VkQueue queue, const VkPresentInfoKHR* pPresentInfo);
