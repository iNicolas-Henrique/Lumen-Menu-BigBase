...
namespace YimMenu
{
	...
	bool Renderer::InitVulkan()
	{
		...
		VkPhysicalDevice FallbackGPU = nullptr;
		for (const auto& Gpu : GpuArr)
		{
			VkPhysicalDeviceProperties Properties;
			vkGetPhysicalDeviceProperties(Gpu, &Properties);
			if (Properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU && !FallbackGPU)
				FallbackGPU = Gpu;
		...
		{
			VkPhysicalDeviceProperties properties{};
			vkGetPhysicalDeviceProperties(MainGPU, &properties);
	...
	void Renderer::VkCreateRenderTarget(VkDevice device, VkSwapchainKHR swapchain)
	{
		...
		if (uImageCount > RTL_NUMBER_OF(m_VkFrames))
		{
			LOG(WARNING) << "Swapchain has more images than the Vulkan overlay supports: " << uImageCount;
			return;
		}
		m_VkImageCount = uImageCount;
		...
		}
		{
			...
			const VkExtent2D framebufferExtent = {
			    m_VkImageExtent.width ? m_VkImageExtent.width : static_cast<uint32_t>(*Pointers.ScreenResX),
			    m_VkImageExtent.height ? m_VkImageExtent.height : static_cast<uint32_t>(*Pointers.ScreenResY)};
			...
			info.pAttachments            = attachment;
			info.layers                  = 1;
			info.width                   = framebufferExtent.width;
			info.height                  = framebufferExtent.height;
	...
	{
		if (!GetInstance().m_VkDevice)
			return;
		vkDeviceWaitIdle(GetInstance().m_VkDevice);
		...
		if (GetInstance().m_VkRenderPass)
		{
			vkDestroyRenderPass(GetInstance().m_VkDevice, GetInstance().m_VkRenderPass, GetInstance().m_VkAllocator);
			GetInstance().m_VkRenderPass = VK_NULL_HANDLE;
		}
		GetInstance().m_VkImageCount = 0;
	...
	VkSemaphore Renderer::VkOnPresentImpl(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
	{
		...
		if (!GraphicQueue || pPresentInfo->swapchainCount != 1)
			return VK_NULL_HANDLE;
		...
		for (uint32_t i = 0; i < pPresentInfo->swapchainCount; ++i)
		{
			...
			if (!ImGui::GetIO().BackendRendererUserData)
			{
				...
				init_info.Subpass                   = 0;
				init_info.MinImageCount             = std::min(m_VkMinImageCount, m_VkImageCount);
				init_info.ImageCount                = m_VkImageCount;
			...
			{
				std::vector<VkPipelineStageFlags> stages_wait(waitSemaphoresCount, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
			...
			return fsd->ImageAcquiredSemaphore;
		}
		return VK_NULL_HANDLE;
	}
